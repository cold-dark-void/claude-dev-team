...
sub list below and stop. Remaining args pass through unchanged to the routed
behavior.
...
| `<sub>` | Strategy | Source |
|---------|----------|--------|
| `config` | Inline | former `/memory-config` |
| `distill` | Inline | former `/memory-distill` |
...
2. **Run `/memory validate $VALIDATE_ARGS` now** — follow the **validate** sub
   section below (Steps 1–8). Do NOT pass `--deep` (circular dependency:
