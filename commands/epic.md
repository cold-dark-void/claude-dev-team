...
| `[--autopilot[=<token>]]` | (with decompose/resume) self-answer A.5/B.3 scope gates (SPEC-033 / CDT-111-C4). Flag beats `AUTOPILOT=1` env; `<token>` ∈ {patch,minor,major,master} — release bumps or land-no-release sentinel `master` (token spelling only; land target is worktree baseline / origin default). Unused by `/epic` (never ships) but resolved+carried for seed parity. Mode B keeps walking the ready-set in the same run until: B.3 halt/`n`, empty ready-set, all children completed (then B.7 iff `release_bump`), only blocked/in_progress remain, M13 seed-fail, or user interrupt — not after the first shipped child. B.5 completion is never self-answered (N8). Independent of `--worktree`/`--release`. |
