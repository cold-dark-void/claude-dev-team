...
### v0.29.13
- **Opus 4.8 alignment pass — trim Opus-4.5-era scaffolding** — Full review of agents/skills against current model + harness capabilities. Findings: model wiring already correct (bare `opus`/`sonnet`/`haiku` aliases auto-resolve), and most "defensive" instructions are harness constraints (async-spawn isolation, post-compaction re-read) or process discipline (PM gate, atomic PRs) that remain valid. Changes: (1) `kickoff` no longer pins the Codebase Explorer agent to "Sonnet" in prose — now model-agnostic like the PM/Tech-Lead prompts; (2) the three near-identical "Excuse you might generate → Why it's wrong" tables in `ic4`/`ic5`/`qa` collapsed into tight `### Non-negotiables` bullet lists — same guardrail intent, less token bloat, drops the patronizing "you won't" framing; (3) `council-judge`'s SPEC-013 MUST list tightened (two overlapping strike rules merged) while preserving every requirement and both verdict/finding taxonomies. Intentionally left unchanged: `distiller`'s `NEVER delete` / batch-failure / DB-lock rules (data-safety invariants, not model scaffolding) and `/orchestrate`'s per-task Tech-Lead review (correct for DAG unblocking, not a weak-model artifact; PM+Tech-Lead concurrency is already implemented).
