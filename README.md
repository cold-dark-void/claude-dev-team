...
> **Note**: `/scaffold-project` and `/init-orchestration` generate a local `.claude/settings.json` that pre-approves common operations so agents run without permission prompts. See [Autonomy & Permissions](#autonomy--permissions) below.
...
`.claude/settings.json` is generated locally — by `/scaffold-project` for interactive/solo work, or by `/init-orchestration` for Agent Teams — and is gitignored, not shipped with the plugin. It pre-approves common operations so agents run without prompting for every tool call. The curated allowlist below is what `/scaffold-project` emits for interactive use:
...
- **Intentionally excluded** (curated/interactive path only): destructive commands like `rm` and `wget` still prompt for confirmation. Orchestration mode (`/init-orchestration`) instead grants `Bash(*)` under `bypassPermissions`, where the OS sandbox — not the allowlist — is the boundary.
...
Check the plugin into your project's settings so teammates get it automatically. The `.claude/settings.json` generated by `/scaffold-project` / `/init-orchestration` is gitignored, so add the marketplace entry to a settings file you commit (the generated permissions block is shown for reference):
