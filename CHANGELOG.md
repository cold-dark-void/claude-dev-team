...
- **Command and skill Surface layering** — thin `commands/*.md` front doors so hosts that only load `commands/` (opencode and other TUIs) can invoke engines that already shipped as skills. Not a new Surface: bump-class now treats `commands/<name>.md` as a host adapter when `skills/<name>/SKILL.md` already exists on the old ref. Patch.
- **CDT-247 command-versus-skill rule in AGENTS.md** — Commands and skills section: a command parses/gates/resolves then reads or spawns the engine; a skill is the engine and carries `user-invocable: false`; a user-facing skill without a command file is a defect. Names the four SKILL.md-less libraries.
- **CDT-248 `user-invocable: false` on 24 engine/protocol skills** — they drop out of the `/` menu. README `/ci-watch` documented as armed by `/orchestrate`, not user-typed.
- **CDT-249 wire `/craft-loop`** — command path-resolves `skills/craft-loop/SKILL.md` (bug-hunt PDH stanza). Skill flagged.
- **CDT-250 eight thin command doors** — `orchestrate`, `kickoff`, `release`, `refactor`, `wrap-ticket`, `backlog`, `review-and-commit`, `brainstorm`. Skills flagged. Not ci-watch.
- **CDT-251 bump-class counts new unflagged skills as surfaces** — flagged `SKILL.md` stays patch-eligible.
- **CDT-257 `/doctor` WARN `skills.user_invocable`** — leaking skill-only SKILL.md; `--fix` allowlist unchanged.
- **CDT-258 static tests for seven protocol skills** — kickoff, refactor, debug, spec-tooling, review-and-commit, brainstorm, fix-ticket (126 checks).
- **CDT-255 thin `/council` command; skill is SoT** — `commands/council.md` becomes parse / preset / PDH (bug-hunt stanza) / Read skill (under 150 lines). Protocol, substitutions, and § 1.5 tiering live in `skills/council/SKILL.md` (`user-invocable: false`). Four existing council tests unchanged.
- **CDT-254 move `/spec` subcommands into `skills/spec-tooling`** — `check`/`create`/`find`/`list`/`update` leave `commands/spec.md` for sibling partials; generate/tests/reflect stay in `SKILL.md`. Command is parse + PDH (bug-hunt stanza) + Read skill (under 150 lines). Skill stays `user-invocable: false`. No plugin.json bump (epic seal is the patch).
- **CDT-253 slim `/memory` to a dispatcher over engine skills** — move config/distill/export/stats into `skills/memory-store/{config,distill,export,stats}.md`, search `--status` into `skills/memory-recall/SKILL.md`, and the validate host pipeline into `skills/validate-memory/pipeline.md`. `commands/memory.md` is parse + PDH + Read skill (target under 150 lines). Four engines stay `user-invocable: false`. Characterisation locks `/memory stats` gather and `/memory search` (`--status` + keyword fallback) against a fixture DB. No plugin.json bump (epic seal is the patch).
- **CDT-252 extract /retro engine into skills/retro** — move the protocol (discovery, filters, phase-1 gate loop, phase-2 spawn, routing, trial review, confirm/apply) from `commands/retro.md` into `skills/retro/SKILL.md` (`user-invocable: false`). Command is the thin front door (usage, flags, PDH, Read skill). Characterisation tests lock gate-path stdout against fixture sessions. `retro-gate` and `retro-subagent` stay. `check-bump-class.sh` no longer SIGPIPEs on a large flagged skill (pipefail 141). No plugin.json bump (epic seal is the patch).
- **CDT-256 docs-drift surface-readme and surface-skill** — README backticked `/name` tokens (outside the renamed/removed table) must resolve to `commands/<name>.md` or an invocable skill; every skill without `user-invocable: false` must have a matching command file. Same exit contract; `/release` Step 4.9 and CI pick it up automatically. `ci-watch` is `user-invocable: false` (not a user-typed Surface). No plugin.json bump.
- **CDT-250 thin command front doors for 8 skill-only surfaces** — add `commands/<name>.md` for orchestrate, kickoff, release, refactor, wrap-ticket, backlog, review-and-commit, and brainstorm (PDH stanza + Read `$SKILL` + `$ARGUMENTS`; protocol stays in the skill). Set `user-invocable: false` on those eight skills. Not ci-watch. No plugin.json bump (epic seal is the patch).
- **CDT-259 name skills/ library exceptions; keep focus/blunt backends** — `skills/README.md` is the skills-dir local index. It names the four SKILL.md-less libraries (`plugin-dir.sh`, `worktree-lib.sh`, `agent-memory/`, `notify/`) and points at AGENTS.md Commands and skills (SoT; not duplicated). Keep `skills/focus` and `skills/blunt` (`user-invocable: false`); they are live `/mode` backends, not empty stubs. Planned removal: **v1.19.0**, after `/mode` inlines the protocol. Deleting them this patch would break `/mode` skill-delegate. Docs only; no plugin.json bump.
...
### v1.18.9
- **Copy-extract IC rule (escalation, not license)** — ICs were shipping a second copy or another `if` on an existing axis because `ic4.md` said follow existing patterns / never invent abstractions, and `ic5.md` said refactor is a separate PR. Extracting a this-diff copy is now matching the codebase, not a new license: keep the IC4 opener and Do-NOT; if the extract needs a new pattern, flag Tech Lead first. Closed waivers, same shape as kickoff `WHY INLINE REJECTED`: `COPY-ACCEPTED: divergence-expected` or `EXTRACT-DEFERRED: pre-existing-dup`. Omit = extract. Unknown text is not a waiver. A false reason still fails. Decision-time tables (`ic4.md:70–71`, `ic5.md:84`, `ic5.md:114`) carry the this-diff vs pre-existing carve-out. Rule lives in `agents/*.md` so `/debug patch` and `/fix-ticket` see it. Kickoff Step 6 and orchestrate `06-design.md` emit an optional plan-level `## Copy-extract` field (Tracking dual-home, no shared partial). One enforcement: Step 9 Evaluate item 4. No Step 8 spawn addendum, no QA/Step 10 gate, no metrics hook, no new `commands/*.md`. SPEC-003 Role Boundaries MUST + Test greps. Edits outside managed includes. Patch (v0.29.12 class — standing-rule tables, no new Surface).

### v1.18.8
- **CDT-235 sync-includes mode dispatch fails closed** — `skills/agent-memory/sync-includes.py` read `mode = args[0] if args else 'check'` with no validation, so any unrecognised mode fell through to the `else` branch and printed `apply: rewrote 0 region(s).` at exit 0. It did not merely skip the check: it reported an apply that never happened. `--check` — the form every sibling tool in this repo accepts, and so the natural thing for someone to "fix" a call site to — was therefore a silent green. That script is the only gate protecting the seven managed `## Persistent Memory` regions in `agents/*.md` from drifting off `skills/agent-memory/protocol.md` (SPEC-004:28-29) and is a blocking pre-commit gate, so a vacuous pass there ships agent-copy drift. Unrecognised modes now exit 64 with usage on stderr, matching the posture `check-staged-paths.sh` and `plugin-dir.sh` already take; `-h`/`--help` prints usage on stdout at exit 0; `check`, `apply`, and the bare no-arg default are byte-identical to before, verified by running the pre-fix and post-fix scripts side by side. A second fail-open in the same parser is closed with it: `--root` with no following value raised an unhandled `IndexError` at rc 1 — this repo's *drift* signal — so a typo in a gate invocation was indistinguishable from real drift and arrived as a traceback; it now exits 64 like any other usage error.
- **The regression test proves the contract, not two examples** — the first version asserted that the two literal strings `--check` and `bogus` were rejected, and passed 12/12 while an allowlist widened to `('check','apply','applesauce')` restored the original defect verbatim: `applesauce` printing `apply: rewrote 0 region(s).` at exit 0. A suite that stays green while the bug it exists to prevent is back is the same failure one level up. `skills/agent-memory/sync-includes-test.sh` is new (the directory had no test file at all) and is now table-driven over 14 unknown-mode shapes — flag-like, case variants, near-misses, trailing space, empty string, path-like — plus `--root` in four positions including the missing-value crash, and asserts stdout stays empty on every rejection so a usage error cannot be swallowed by a caller redirecting only stderr. PASS 12 → 58; the allowlist widening and the guard removal each take it red, confirmed independently by the implementer, QA, and the orchestrator using three different mutations.
- **Managed-region drift is now caught on PR, not only at release** — `sync-includes` appeared nowhere in `.github/workflows/`, so the seven agent regions were gated at release time only, asymmetric with `plugin-dir-test.sh`, which CDT-232 deliberately wired into both gate sets. An `include-drift` sibling job joins `smoke.yml` (8 jobs → 9), no `needs:`, so it reports independently. It was verified capable of failing before being trusted: drift injected inside a managed region makes it exit 1 and name the file, agent, and partial with a diff. A caution recorded from getting this wrong twice — the `## Persistent Memory` heading sits *outside* the `<!-- include: -->` markers, so editing it changes real bytes and is correctly ignored; a mutation that applies is not the same as a mutation that is in scope, and the check that closes the gap is confirming the mutation reproduces the original defect. Script, test, and CI only; no product behaviour changed ⇒ patch.

### v1.18.7
- **CDT-233 stanza irreducibility verdict, recorded executably** — the PDH bootstrap stanza's per-site duplication has now been challenged three times, each attempt re-deriving the same reasoning because the verdict only ever existed as prose. SPEC-002 gains a `Stanza irreducibility — CDT-233 verdict` section dispositioning all three proposed escapes with mechanisms rather than preferences, and `skills/plugin-dir-test.sh` gains five assertions (PASS 184 → 189) that a future ticket cannot quietly contradict. The newly-raised escape is the interesting one: a stanza *version tag*, where an outdated installed copy is expected to **decline** an unrecognised tag rather than answer with its own possibly-stale root. That form is genuinely **not** circular — unlike a shipped bootstrap file or a `--self` mode, both of which need the path they would resolve — so the standing circularity objection does not reach it. It dies on a verified mechanism instead: `skills/plugin-dir.sh:430` dispatches `case "$sub"` on `$1` alone and never validates trailing argv, so every already-shipped copy silently ignores an unknown tag and exits 0 with its own root. The refusal the scheme depends on is precisely what an old copy cannot perform, so a stale copy still answers authoritatively — the CDT-82/CDT-166 failure shape reached through a new door. Second, independent kill: refusal is only safe if the caller also retains the full inline ranker for when refusal fires, making the stanza find-any + delegate + refusal-detect + inline fallback — strictly longer than today, self-defeating for a ticket whose premise is token cost. The counter-nuance is recorded rather than buried: a *modern* copy planted at a stale path self-corrects, so delegation would be safe exactly when every reachable copy is modern, which cannot be known without asking, and asking returns 0 either way.
- **The verdict is now two executable proofs, not a paragraph** — V1 plants a two-root cache and asserts that a find-any-then-delegate bootstrap resolves the stale root while the canonical stanza resolves the correct one; V2 is a re-open tripwire asserting `plugin-dir.sh root --pdh-contract=2` still exits 0, failing with a message that names the verdict it would invalidate. V2 is deliberately a tripwire and not a prohibition: it does not forbid hardening argv, it forbids hardening it while leaving a dependent verdict standing. Modelling the delegating bootstrap with `find … | head -1` was rejected after it passed for the wrong reason — readdir order returned the *correct* root first on this filesystem — so it uses `sort | tail -1`, which is also the forbidden pre-CDT-166 spelling and therefore an honest model of the defect.
- **Trim measured, then declined on value** — the comment/waiver trim that motivated the original ticket was fully prototyped, all gates green, no red window, and then not shipped: it saves 23,375 B, which is 15.0% of the stanza payload but 1.2% of the ~1,978,055 B prompt corpus, and its waiver half additionally required a `lint.py` rule change argued against SPEC-021's standing "do not weaken the check" MUST. The real lever is the CDT-166 version-segment ranker at ~52% of the 746 B line, and the verdict above is precisely the finding that it cannot be removed safely. SPEC-002 records the decision with its re-derivation commands and its flip condition, so the fourth attempt starts from the measurement instead of re-deriving it. Also corrected there: the long-standing claim that every site emits one comment line plus one `PDH=` assignment was already false and ungated — the comment appears at 22 of 209 emissions.
- **A tripwire nearly killed by its own fix** — the harness's stale-copy fixture first composed its `*/dev-team/*` glob from shell variables to keep CDT-234's discovery-chain counters byte-stable. That made SPEC-002's own designated flip test non-empty for the first time, on the shape the spec calls "the important one" and instructs reviewers to treat as blocking — trading a declared blocking invariant for numbers the same document declares "a dated measurement, not a contract" three bullets above. The glob is now spelled literally, the harness stays inside the discovery predicate and is caught by the existing harness-self exclusion rather than a new carve-out, and the chain is re-derived to `232 − 219 (209+7+2+1) − 1 = 12` with the gated `12` unchanged. Documentation and tests only; no product file, emitter, or gate behaviour changed ⇒ patch.

### v1.18.6
- **CDT-237 stanza branch-0 and branch-1 coverage** — every one of the five `$STANZA_SH` invocations in `skills/plugin-dir-test.sh` ran `env -u CLAUDE_PLUGIN_ROOT` from a foreign cwd, so branches 0 (`CLAUDE_PLUGIN_ROOT`) and 1 (dev-checkout cwd) — 183 B of the 745 B stanza, replicated at ~210 emission sites — could be edited, broken, or deleted outright while the harness reported `PASS=176 FAIL=0`. The one positive `CLAUDE_PLUGIN_ROOT` test exercised the `plugin-dir.sh` CLI, not the stanza. Eight new assertions (PASS 176 → 184) close it: branch-0 positive and guard-negative, branch-1 positive, and full precedence 0 > 1 > {2, 3}, all comparing the resolved path via `assert_eq` against the stanza extracted from SPEC-002 at runtime. The 1 > 2 link matters most — a `swap12` mutant that reorders the stanza to prefer an installed marketplace clone previously passed every assertion in the file, because all six stanza call sites `cd "$FOREIGN"` and the harness asserts `$FOREIGN` holds no `skills/plugin-dir.sh`, leaving branch 1 unsatisfiable everywhere. That is the CDT-82 shadow class returning through a hole in the test rather than in the code. Two further mutants that slipped through are also now caught: an unquoted `$CLAUDE_PLUGIN_ROOT` (branch 0's whole job is consuming a user-supplied env path, so a planted root containing a space asserts the quoting) and `pwd` → `pwd -P` (the old compare was structurally incapable of seeing it; the checkout is now planted behind a symlink). Test-only; no product file and no spec text changed ⇒ patch.
- **Each new assertion proven to bite** — the implementer, the orchestrator, and QA each mutated the canonical stanza independently and confirmed every new assertion fails for its own stated reason and names the wrong resolved path. Adding a test nobody has watched go red is what produced five silent passes in v1.18.5; a green run from a mutation that did not apply is the same defect one level up, so the mutation harness asserts the edit landed before trusting the result. Residual gaps are recorded rather than implied: `set -u` safety of the stanza is untested at all 11 call sites (CDT-238), and branch-2/branch-3 internal mutants stay out of scope here.

### v1.18.5
- **CDT-166 ranking generalised to all cache resolvers** — CDT-166 established that plugin cache hits must be ranked by the `/dev-team/<VER>/` path *segment*, but the fix only ever landed in the PDH bootstrap stanza, where skill-lint C5 enforces it by byte-identity. Twelve other resolvers sit outside C5 entirely and still ranked by sorting full absolute paths, so the install-channel slug (which precedes the version segment) decided the winner: `…/cache/zzz-other/dev-team/2.0.0/…` beat `…/cache/cold-dark-void/dev-team/10.0.0/…` and any user with a lexically-higher slug silently resolved a stale engine. The tilde map was present and correct at every one of those sites and did not help — a tilde-mapped full-path `sort -V` is the defect, not a mitigation of it. All 12 are ported to the `awk -F/` segment ranker: 9 single-line `embed-one.sh` locators (fixed once in `skills/agent-memory/protocol.md` and machine-expanded to the 7 `agents/*.md` managed-include regions per SPEC-004, plus the `skills/wrap-ticket/SKILL.md` hand-copy that `sync-includes.py` cannot see) and 3 multiline hook-runtime bootstraps, which keep their `_pdh_hit=` assignment shape because collapsing them into a `PDH=$(` line would newly trip C5 with an obligation detached hooks cannot satisfy. Their `lint-ok: C5` waivers now state on their face that the exemption covers byte-identity only; SPEC-002 records the general rule that a drift exemption is never a correctness exemption.
- **Cache-resolver coverage gate** — `skills/plugin-dir-test.sh` gains an extract-and-execute gate (PASS 55 → 176), already blocking via `smoke.yml` and release Step 4.12. A grep gate cannot do this job: the existing CDT-53-13 tree gate targets a *bare* `sort -V` and explicitly skips tilde-mapped lines, so it is structurally blind here, and 3 of 12 sites are multiline. The gate instead discovers resolvers mechanically, extracts each one from the product file at runtime, and executes it against a synthetic `$HOME`; it fails loudly on zero found, on any count mismatch, and on extraction failure, and prints a full audit chain (`231 predicate matches − 218 mechanical − 1 named = 12 executed`) so exclusions are visible rather than implicit. A permanent negative proof asserts the *tilde-mapped* defective form still resolves `/2.0.0/`, which is the discriminator CDT-232's withdrawn task lacked — it planted a bare `sort -V`, exercised pre-existing capability, and went green while proving nothing.
- **Five over-claims caught by execution, not review** — this ticket's own artifacts falsified themselves five times, each time as a silent pass rather than a crash: the spec's multiline extraction anchor keyed on backslash continuation, which breaks on the embedded awk program the port itself introduced; the discovery predicate keyed on the literal cache-root spelling, so a variable-rooted resolver carrying the defect verbatim scored `discovered 12 / PASS=176 / exit 0` in review; the family classifier re-imported that same spelling one bullet later; spec and harness disagreed on the discovered count; and the new audit chain's own fixtures filter carried a leading slash that never matched, mis-bucketing two lines. Prose review passed all five. SPEC-002 now records the pattern — prefer a mandated structural invariant over a lexical predicate, exclude by name with a rationale rather than by loosening the predicate — and states the gate's remaining reach boundary (a `-path` argument built from a variable still evades it; confirmed empty in-tree by three greps, with the flip condition written down) instead of claiming completeness.

### v1.18.4
- **Handoff miner summary citation trap** — the optional wrapper `summary` (CDT-201) is validated all-or-nothing: one sentence without a `{<id>}` token discards the *entire* summary, so `assemble` emits `omit summary: uncited sentence` and the packet loses its `### Where we are` block. Two consecutive real captures hit this the same way — every substantive sentence was correctly cited, but a short uncited scene-setting opener (`"Session spans three arcs."`) killed an otherwise-valid 369-char summary. Root cause was prompt under-specification, not miner disobedience: SPEC-018 already requires static negative+positive examples for the other two trap rules (bare-pick rulings, `product_surface`), and the summary rule was the only one without them. Both merged-miner templates (`skills/handoff/SKILL.md` and `skills/handoff/LIGHT.md` — light never reads SKILL.md, so each needs its own copy) now spell out the all-or-nothing consequence, the `(?<=[.!?])\s+` split with trailing fragments counting as sentences, and the uncited-opener failure mode, with a matched NEGATIVE/POSITIVE example pair. `light-static-test.sh` gains two assertions that both files carry it, matching the existing T11 guard so the examples cannot be silently dropped. SPEC-018's miner-prompt clause gains (f) recording the requirement and its rationale. The validator and its fail-closed behaviour are deliberately unchanged — a framing sentence is exactly where a fabricated root cause would hide, so salvaging partial summaries would trade a cosmetic annoyance for a hole in the invent-guard; clause (f) says so explicitly to stop a future reader "fixing" it there. Prompt, spec, and test only; no assemble behaviour change ⇒ patch.

### v1.18.3
- **CDT-232: PDH stanza harness wired into blocking gates** — `skills/plugin-dir-test.sh` proves the SPEC-002 bootstrap stanza's fallback branches resolve (marketplace clone, version-ranked cache) — the code path only *installed* users take, since a dev checkout short-circuits at branch 1. It ran in no gate at all. Now wired as a sibling job in `.github/workflows/smoke.yml` and as pre-commit gate Step 4.12 in `skills/release/SKILL.md`. Three previously-untested invariants gained assertions: branch-1 bypass (was implicit), equal-`<VER>` `cold-dark-void` preference at stanza level (was proven only via `plugin-dir.sh`), and the empty-`$PDH` fail-mode that SPEC-002:142's "introduces no new failure path" claim rests on (asserted in prose since it was written, never tested). The harness now **extracts the canonical stanza from SPEC-002 at runtime and executes that exact text** instead of validating a hand-copied re-quoted duplicate — closing a gap where the two gates did not compose: skill-lint C5 proved all 207 emission sites match canonical, the harness proved *a copy* resolved, and nothing proved canonical itself resolved. It fails loudly if the canonical block is not extractable, so the gate cannot go vacuous. Also fixes a latent `rm -rf /home`: the harness had `set -u` but not `set -e`, so a failed `mktemp -d` left `$TMP` defined-but-empty and six `rm -rf "$TMP/home"` sites would have resolved to `/home` — now guarded twice (exit 70 pre-trap, plus a prefix check on every deletion), fixed *before* the harness reached CI. Two runbooks carrying the pre-CDT-166 full-path `sort -V` ranker that SPEC-002:141 marks MUST NOT are corrected, prose included (`migrate-to-v1.md` had been *prescribing* the forbidden arm as the remedy). SPEC-002 gains the verification-gate MUST and an evidence correction: a prior audit's "3 stanza variants" was not drift — it was one SPEC-021-documented exemption plus `lint-ok: C5`-waived hook-runtime bootstraps; corrected counts are 50 files / 207 emissions / 1 variant. `docs/` regression gating was attempted, found to catch nothing (the gate skips tilde-mapped lines; the docs defect *is* tilde-mapped), and withdrawn rather than shipped green-but-toothless — tracked as follow-up along with 12 non-PDH cache resolvers that still rank by full path. Behavior-preserving; no emission site changed ⇒ patch.

### v1.18.2
- **Cost-optimization phase 2: pin unnamed general-purpose spawns to haiku** — four `general-purpose` Task spawns outside the model-map's named-roster contract were inheriting the parent session's model tier by default: `/retro`'s per-session friction-analysis spawn (`commands/retro.md`), and `/memory validate`'s claim-extractor, Tier B investigator, and pair-judge spawns (`commands/memory.md`). All four are bounded, rubric-driven batch classification with command-enforced output validation downstream (malformed output degrades to a safe default rather than being trusted), the same shape of work `/handoff`'s chunk-summarizer already runs on haiku — so pinning them extends an already-shipped precedent rather than introducing a new one. `skills/model-map/SKILL.md`'s "omit the fence" list clarified: omitted from the named-roster map does not mean unpinned. Behavior-preserving cost hardening ⇒ patch.

### v1.18.1
- **model-map dogfood fixes (CDT-231)** — a bug-hunt run against `skills/model-map` (fresh off CDT-230, using the new `finder` agent for discover+refute) confirmed 8 findings. Fixed 5: `skills/model-map/SKILL.md`'s spawn-surface coverage table corrected from `ic5`/`ic4` to the actual `finder`/`debugger` primary agents (was a live SPEC-037 M17 violation) and gained the missing `finder`/`debugger`→`ic5` host-reject fallback line; its `write-model.sh list` agent count corrected from stale "8" to the actual 10; `resolve-model.sh`'s unparseable-JSON / jq-missing / unknown-agent warnings are now field-aware (`--effort` mode says "using inherited effort", not "using Tier default" — `effort-test.sh` updated to match); `write-model.sh`'s local writer (`set`/`set-effort`/`unset`/`unset-effort`) now serializes its validate-then-read-modify-write sequence under `flock`, closing a TOCTOU window where two concurrent writes could silently clobber each other. One finding (roster/allowlist helpers duplicated verbatim across `resolve-model.sh` and `write-model.sh`) confirmed real but deferred — a shared-source-of-truth extraction is a larger structural change than warranted for a patch. Two findings needed no fix: a "prompt-injection vector" framing was refuted by adversarial review (the value only reaches a schema-constrained Agent tool parameter); a `qa`/`council-judge` WARN-never-FAIL characterization was confirmed accurate but intentional per-spec design (SPEC-037 M9), not an oversight. Report: `.claude/bug-hunt/2026-08-31-model-map.md`. Behavior-preserving hardening ⇒ patch.
