...
### v1.18.15
- **Full project review & enhancement proposal (docs only)** — `docs/reviews/2026-09-23-full-review/` records a read-only, every-file review of v1.18.14 split into ten slices (agents/infra, small commands + docs, large commands, council + fix-ticket, orchestration, handoff/transcript/retro, memory/model-map, workflow skills, release/tooling, specs). `README.md` consolidates 20 verified P0 defects (among them a reproduced memory-DB SQL injection in `reconcile-lib.sh`, `git clean -fd` in epic seal, a wrong-ref remote prune, `\ # lint-ok` line-continuation breaks, cross-fence shell state in `/retro` and `/setup team`), the test baseline (5 suites red on master, ~9 of 76 test scripts gated in CI), eight root-cause themes, and a wave-ordered roadmap with suggested ticket slicing. No product file changed ⇒ patch.
