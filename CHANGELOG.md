...
- **Wired into CI and `/release`** — new `all-tests` job in `smoke.yml` (existing job ids unchanged) and `/release` Step 4.13 "All-suites gate". Contract in SPEC-030 R1–R17; SPEC-010 points to it.
- **Smoke harness widened** — `tools/smoke/smoke.py` now checks `agents/*.md` (the five SPEC-003 keys; `model` and `effort` values), fence-only sub-docs, `bash -n` on every tracked `*.sh` (tests included) and every `githooks/` file, and accepts YAML block lists. Classification uses the repo-relative path. New fixtures and `tools/smoke/README.md`.
- **Adoption fixes** — five placeholder fences marked `bash template` (`autopilot/end-state.md`, `orchestrate/steps/08-execute.md`, `09-review.md`); real heredoc-terminator bug fixed in `agents/project-init.md`; `docs-drift/test.sh` live-tree bites now use a scratch copy and never write to the checkout.

### v1.18.15
- **Prompt audit: 47 fixes across 46 prompt-surface files** — agents, commands, skills, `AGENTS.md`; full findings in the audit report (not committed). Real bugs: `AGENTS.md` session-start memory read tested `$MEMDB` before assigning it, and `/setup` Steps 2.5/4 built `MEMDB` from an unset `$MROOT`, so SQLite memory silently never loaded on those paths — assignments reordered. Pinned `Co-Authored-By: Claude Opus 4.8` example (6 files) replaced with the model-agnostic form. Build-phase archaeology that contradicted live behaviour removed: `self-answer.md` no longer claims it has no callers (orchestrate/kickoff invoke it), `/focus` / `/blunt` no longer described as deprecation-stub commands that do not exist, council-judge no longer branches on a cortex that is never injected, `/spec` no longer waits on "Task 12" stubs. Contradictory duplicates reconciled: claim/plan extractors now quote a span of INPUT_TEXT (paraphrase clause dropped to match the validator), tier-triage asks about numstat not diff content, release Step 3 no longer says "never skip" beside skip-if-present, debug same-fix-many-places vs sibling-hits rules scoped. Pressure-language trims (CRITICAL banners, "not to be nice", "err on the side of asking", excuse-table jabs) reworded with the reason kept; `EXTRACT-DEFERRED`/`COPY-ACCEPTED` tokens and all test-grepped MUST lines unchanged. Stale facts: fix-ticket version-pair list, transcript-parse thinking-join contract, PM "escalate to Opus" (it is Opus). Patch.
