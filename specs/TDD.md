...
| SPEC-021 | Skill-Bash Lint Gate | DRAFT | skills/skill-lint/check-skill-bash.sh, skills/skill-lint/SKILL.md, skills/skill-lint/fixtures/, skills/release/SKILL.md |
...
| 2026-07-03 | SPEC-021 created: Skill-Bash Lint Gate — deterministic LLM-free linter for fenced bash blocks (C1 cross-block var scope, C2 zsh history-expansion hazard, C3 zsh-fatal unguarded glob, C4 captured inline-PRAGMA sqlite poison), per-line lint-ok waivers, /release pre-commit gate wiring, fixture bite-tests; conflict-scanned (no blockers; SPEC-010 gate-host + SPEC-008 format-check overlaps resolved by precedent/exclusion). ID 020 is allocated to /craft-loop on feat/spec-020-craft-loop |
