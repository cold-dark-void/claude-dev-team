...
- **M2 — Full check battery.** A single run MUST execute at least these check groups: **(a) version pair** — `CHANGELOG.md` latest `### vX.Y.Z` heading vs `plugin.json` `version`, evaluated against the *resolved* plugin install (dev checkout or cache) per SPEC-002's rule (`marketplace.json` is not version-synced); **(b) memory stack** — `sqlite3` presence, `memory.db` existence, `schema_version` value vs the current expected schema, optional extensions (sqlite-vec / lembed) loadable, and embedding-config coherence (mode `remote` requires a non-empty `EMBEDDING_URL` and its host in the sandbox network allowlist; mode `lembed` requires extension + GGUF present); **(c) hooks wiring** — project `.claude/settings.json` hooks vs the canonical set `/setup orchestration` emits from init-orch **templates**, plus **managed-only** hook hygiene (M2c″: managed commands `${CLAUDE_PROJECT_DIR}`-anchored, no pipe operators, referenced scripts exist and are executable **after setup**); **(d) settings sanity** — valid JSON, `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` present when team memory is initialized, and coherence warnings (e.g. `bypassPermissions` or shipped `dontAsk` with sandbox disabled); functional sandbox runtime probe (check id `settings.sandbox_runtime`): when `sandbox.enabled` is true, execute a ≤3s offline bwrap minimal-init+`true` probe; WARN never FAIL if init fails or bwrap absent; SKIP when sandbox not enabled, settings unparseable/absent, or non-Linux; nested-session probe failure MUST WARN with nested caveat (MUST NOT silent PASS/SKIP); stronger detail when `defaultMode` is `bypassPermissions`|`dontAsk`|`auto`; MUST NOT write project state; **(e) optional dependencies** — `jq`, `python3`, `gh`, each reported with a named per-feature impact statement (e.g. "no gh → ci-watch / gh-backed release steps unavailable"); **(f) worktree state** — stale `.worktrees/` entries, `.wt-lock` FRESH/STALE per SPEC-016's TTL rule, orphaned locks (lock without a registered git worktree and vice versa), and a held `distilling_lock`; **(g) plugin cache resolution** — resolve a known relpath via `skills/plugin-dir.sh` and report which tier answered (dev checkout / versioned cache / find-fallback) or its not-found exit 3.
- **M2c′ — Hooks expected after setup; never FAIL for untracked package hooks (CDT-54).** Hook presence/executability checks apply to the **consumer project path** (`.claude/hooks/*` under the resolved project root) **after** `/setup orchestration` (or equivalent emit). Absence of live hooks when orchestration has never been run MUST surface as WARN (or FAIL only when settings **wire** a hook that points at a missing script — same M3 hard-invariant rule). Doctor MUST **NOT** FAIL solely because package-tracked live hooks under the plugin repo's `.claude/hooks/` are missing (live hooks are generated+gitignored, not package product). Dual-copy template-vs-live drift against tracked package hooks is not a FAIL class.
- **M2c″ — `hooks.hygiene` is managed-only (CDT-77).** Hygiene findings (unanchored path, pipe operator, missing script, non-executable script) apply **only** to **managed** hook commands. A command is managed iff it references `.claude/hooks/<basename>.sh` where `<basename>` (without `.sh`) is in the expected-script set derived by `parse_expected_hooks` from init-orch templates (`EXPECTED_HOOK_SCRIPTS` — same SoT as `hooks.events` / M9). User-owned commands (pathless tools like `go vet` / `cat AGENTS.md`, or `.claude/hooks/<name>.sh` whose basename is **not** in that set) MUST NOT produce WARN/FAIL under `hooks.hygiene` and MUST NOT receive a `/setup orchestration` fix-it. Setup fix-its remain only for managed findings that setup can remediate (honesty with M4). Severity order for managed findings is unchanged: missing FAIL → nonexec FAIL → pipe WARN → unanchored WARN → PASS.
- **M3 — Optional-dep absence is WARN, never FAIL.** Severity semantics: FAIL is reserved for broken hard invariants (version-pair drift, unparseable plugin/settings JSON, `schema_version` mismatch, a wired **managed** hook pointing at a nonexistent script). The absence of any OPTIONAL dependency or opt-in feature MUST surface as WARN accompanied by its impact statement — an otherwise-healthy project missing all optional deps MUST exit with zero FAILs.
- **M4 — Fix-it line per finding.** Every WARN and FAIL row MUST carry exactly one concrete, copy-pasteable remediation — a command (`/setup team`, `/setup orchestration`, `/release`, `/memory distill --force`, `bash skills/worktree-lib.sh release <slug>`) or a one-line instruction. Never a vague "check your configuration". `/setup orchestration` fix-its for hook hygiene MUST only appear when the finding is on a managed command (M2c″) — setup deliberately leaves user-owned hooks alone.
...
- **M6b — Callers MAY gate on exit code (CDT-51).** Bootstrap and other install surfaces (SPEC-005 `/setup team` / `/setup orchestration`) MAY hard-gate on doctor exit codes — treat exit ≤ 1 as proceed and exit 2 as block. Gating is the **caller's** responsibility; doctor remains a pure diagnostic (see M8). Marketplace install MUST NOT hard-gate. Soft-advisory callers (e.g. `/setup project`) MAY recommend doctor without blocking. Bootstrap callers that hard-gate (SPEC-005) MUST invoke doctor with `--gate` matching the sub being run so M6c self-remediation applies; bare doctor stays the strict diagnostic.
- **M6c — Gate-mode self-remediation (CDT-67).**
  `doctor.sh` MUST accept `--gate=<orchestration|team>` (exact tokens; other values → exit 64).
  When `--gate=G` is set, for each FAIL row whose fixit string, after trimming leading/trailing
  whitespace, equals the gate command string C(G) exactly — where C(orchestration)="/setup
  orchestration" and C(team)="/setup team" — that row is self-remediating:
  - status MUST remain FAIL (MUST NOT demote to WARN/PASS/SKIP)
  - human output MUST annotate the row as waived under the active gate
  - JSON MAY add optional boolean `gate_waived=true` on the check object (additive; omit or
    false when not waived / bare run)
  - self-remediating FAILs MUST NOT contribute to exit 2
  Exit under `--gate`: 2 iff ≥1 non-self-remediating FAIL; else 1 if ≥1 WARN or ≥1
...
2. **Version drift (M2a):** against a fixture where `plugin.json` and `CHANGELOG.md` versions differ → FAIL row naming the version pair and the mismatched values, fix-it referencing `/release` (dev) or plugin update (consumer); exit `2`. Missing or present `marketplace.json` `plugins[].version` MUST NOT alone cause FAIL.
3. **Uninitialized memory (M2b, M3, M4):** in a project that never ran `/setup team` → memory checks WARN (not FAIL) and the fix-it line is `/setup team`; exit `1`.
...
5. **Hook drift (M2c, M2c′, M2c″, M9):** remove the `TaskCompleted` entry from `.claude/settings.json` → FAIL naming the missing hook with fix-it `/setup orchestration`; a **managed** hook wired as `bash .claude/hooks/<expected>.sh` (worktree-unsafe, un-anchored) → WARN citing the `${CLAUDE_PROJECT_DIR}` rule with fix-it `/setup orchestration`; user pathless hooks (`go vet`, `cat AGENTS.md`) alongside clean managed hooks → `hooks.hygiene` PASS (no permanent WARN); user custom `.claude/hooks/custom.sh` (basename ∉ expected set) missing/unanchored → no hygiene FAIL/WARN and no setup fix-it; changing the canonical set in `skills/init-orchestration` templates changes `/doctor`'s expectation with no edit to `/doctor` itself; package missing tracked live hooks alone → no FAIL (M2c′).
6. **Optional deps (M2e, M3):** run with a PATH stripped of `jq`, `python3`, `gh` → three WARNs each carrying a named feature impact, zero FAILs on an otherwise-healthy project; exit `1`.
...
12. **Never bootstraps (M8):** run default and `--fix` modes in a bare project → no `.claude/memory/`, no `memory.db`, no hooks created; the report recommends `/setup team` / `/setup orchestration` instead.
...
- [x] All optional deps absent → zero FAILs (WARN-with-impact only)
...
- **SPEC-005** — Team Bootstrap: `/setup team` + `/setup orchestration` own all creation/repair of memory, extensions, and hooks (`/doctor` recommends, never bootstraps).
