...
- **M3 — Optional-dep absence is WARN, never FAIL.** Severity semantics: FAIL is reserved for broken hard invariants (version-triplet drift, unparseable plugin/settings JSON, `schema_version` mismatch, a wired **managed** hook pointing at a nonexistent script). The absence of any OPTIONAL dependency or opt-in feature MUST surface as WARN accompanied by its impact statement — an otherwise-healthy project missing all optional deps MUST exit with zero FAILs.
