...
- MUST NOT register council roles as persistent team agents (no entries in `agents/`, no cortex, no `init-team` bootstrap)
...
- MUST require Prosecutor to produce a brief listing each claim, the evidence against it, and a requested verdict
- MUST require Devil's Advocate to produce a brief listing each claim, the evidence supporting it, and a requested verdict
...
### Phase 5 — Judgment
- MUST route judgment to a dedicated `council-judge` agent defined at `agents/council-judge.md`; the `council-judge` agent MUST inherit `tech-lead`'s cortex, memory, and directives load path to preserve project context but MUST declare an empty tool allowlist in its YAML frontmatter
...
- When no task id is resolved, the `task_id` field MUST be absent from the report frontmatter and the filename MUST NOT carry a `--<task_id>` suffix
