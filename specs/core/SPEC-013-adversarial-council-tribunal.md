...
- For every council prompt template under `skills/council/prompts/`, that file's own `## Variables` table is the authoritative declaration of its `{{TEMPLATE_VARIABLE}}` contract; `skills/council/SKILL.md` runtime substitution blocks and that file's documented-variables table MUST name exactly the variables declared in each prompt's Variables table — no more (no dead substitutions), no fewer (no unsubstituted placeholders leaking into spawned subagents). `commands/council.md` is the thin host (parse / preset / PDH / Read skill) and MUST NOT carry a parallel substitution contract.
...
- [ ] Index confidence normalization (CDT-181): non-int JSON number max conf floors to int 0–100; index stores int|null only; task-bound finalize does not exit 6 solely for non-int conf; automated test covers float max conf
...
| Date | Change |
|------|--------|
| 2026-09-17 | CDT-255: `commands/council.md` is the thin host (parse / preset / PDH / Read skill). Runtime substitution blocks and § 1.5 tiering live in `skills/council/SKILL.md`. Engine Architecture MUST updated: substitution contract is skill-only. Status stays ACTIVE. |
...
| 2026-08-30 | CDT-230: Investigator removed from the "no entries in `agents/`" MUST NOT and carved out as `agents/finder.md`. The two carve-outs are recorded as **separate clauses on separate grounds** — `council-judge` for structural tool-lessness (what it cannot do), `finder` for a generic investigator body shared across `/council`, `/bug-hunt`, and `/debug ticket` (reuse across engines). `finder` has real read-only tools; the Judge still has none, and the `finder` clause MUST NOT be read as licence to give the Judge tools. Added a follow-on MUST holding the tribunal invariants (read-only, blind, no cortex/memory/directives, council protocol stays in `skills/council/prompts/`). Phase 2 / 2.5 spawn `finder` in place of `ic5`; host-reject falls back to `ic5` (SPEC-003 § Role split). Status stays ACTIVE. |
