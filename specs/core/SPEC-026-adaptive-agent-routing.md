...
- **SPEC-019 (local-agent offload)** owns local-vs-Claude routing mechanics, the wrapper exit-code contract, and all token/cost accounting in `.claude/local-agent/metrics.jsonl`. That file is the **format exemplar** for this ledger (JSONL, append-only, `jq`-guarded, `null`-not-`"unknown"`), and its escalation events are one ledger *source* — but this spec MUST NOT write to that file, restate its cost fields, or duplicate its records. `outcomes.jsonl` carries routing-outcome data only.
...
| OQ4 | Emit when a **(task, agent) stint ends**, with QA counters final — **not** mid-path at Step-9 APPROVE before Step-10. Accepted stint: after Step-10 completes for that task (QA PASS, or QA skipped/N/A with `qa_bounces` finalized). Escalated stint: at hand-off (original agent). Local: at local-agent cap escalation. |
...
- **M4 — Emission at existing stint terminals only (OQ4).** Ledger writes are instrumented at checkpoints that already exist — no new hook events:
  ...
  - **(b) Escalated stint** — at hand-off when SPEC-009 stuck-after-2 or 3+-round deadloop (or local-agent cap) ends the current agent's stint; emit `{outcome: "escalated"}` with counters as of hand-off.
  ...
  - **(e)** local-agent escalations from the Step-8 offload sub-block and `/local-do` emit `{agent: "local", outcome: "escalated"}` (ticket/task_id null for bare `/local-do`).
  - All writes go through one helper (`skills/metrics/emit-outcome.sh`, `jq`-guarded, mirroring `skills/local-agent/emit-orch-metric.sh`).
...
- **M8 — Capability boundaries stay authoritative.** Advisories may only propose alternatives legal under SPEC-003 role boundaries and outside SPEC-019's forbidden-offload set — in practice **ic4 ⇄ ic5**, and Claude-vs-local only for offload-eligible tasks. MUST NOT advise a boundary-crossing routing (e.g. implementation to tech-lead/pm, ambiguous/novel work to ic4, release or QA-gate work to the local agent). The ledger tunes routing WITHIN boundaries; it never widens them.

- **M9 — Failure isolation (never block orchestration).** Emit and read paths are best-effort: `jq` absent ⇒ skip the append with a one-line stderr notice (SPEC-019 precedent); unwritable ledger ⇒ same; malformed JSONL lines ⇒ skipped by the reader; any advisory-computation failure ⇒ silent fallback to static rules. A ledger problem MUST never fail, block, or degrade a task assignment, review loop, or `/local-do` run.

- **M10 — MUST NOT (hard boundaries).** MUST NOT silently reroute (hard restatement of M6); MUST NOT write to `memory.db`, `.claude/council/index.json`, or `.claude/local-agent/metrics.jsonl` (owned by SPEC-004, SPEC-013, SPEC-019 respectively); MUST NOT store review feedback prose, diff content, or any free text in records — enums, counts, ids, and timestamps only; MUST NOT carry token/cost fields (cost accounting stays in SPEC-019's metrics file); MUST NOT implement rollup/dashboard display — rendering `outcomes.jsonl` is CDV-187 / banked `/metrics` duty; this spec owns only the write path and schema.
...
- SHOULD surface a one-line per-agent outcome aggregate in `/standup` output (mirroring SPEC-019's local-vs-Claude standup surface). Deferred if it risks LOC/scope creep on CDV-185.
...
4. **Local source (M4, M10):** run `/local-do` (or emit path) to a cap-hit escalation → `{agent:"local", outcome:"escalated", ticket:null}` appended to `outcomes.jsonl`; `.claude/local-agent/metrics.jsonl` receives only its own SPEC-019 records (no cross-writes).
5. **Stint-end timing (OQ4):** Step-9 APPROVE alone produces **no** ledger line; emit occurs only after QA counters are finalized (accepted path) or at escalation hand-off.
6. **Advisory fires with rationale (M5, OQ5):** seed a ledger where ic4 shows ≥5 samples and escalated rate ≥50% on `refactor` → Step-7 assignment of a refactor-class task prints an advisory containing the rate, the sample count, and the suggested alternative (ic5).
7. **Cold-start silence (M7):** with 3 seeded records (< MIN_SAMPLES), or the ledger deleted, the same assignment prints no advisory text and the `Recommended agent:` line matches the static rule exactly.
8. **Override only (M6, OQ3):** decline or ignore a printed advisory → task routes per static rule; accept it → only then does the `Recommended agent:` line change; no path changes routing without advisory + acceptance.
9. **Boundary guard (M8):** seed data "showing" tech-lead outperforming on impl tasks and ic4 "outperforming" on `impl-novel` → no advisory proposing either routing is ever printed.
10. **Null class (M3):** a TaskCreate description missing the `Task-class:` line yields a record with `task_class: null` that never contributes to any advisory cell.
11. **Fail-open (M9):** remove `jq` / make the ledger unwritable / inject a corrupt JSONL line → orchestration and assignment complete normally with a one-line stderr notice; the reader skips the corrupt line; the advisory falls back silently to static rules.
12. **Hard boundaries (M10):** after a full instrumented run, the ledger contains no free-text feedback and no token/cost keys; the metrics helpers wrote nothing to `memory.db` or the council index.
13. **MVP outcomes (OQ1):** no producer path writes `outcome: "rejected"`.
14. **All-time cells (OQ6):** seeded records of any age all contribute to the cell (no window filter).
...
- [ ] Helpers + orchestrate/local-do instrumentation land with bite-tests green
- [ ] One real orchestration run emits well-formed records from stint terminals (accepted after QA, escalated, local escalation; council_overturns derived from index)
...
| qa/devops/ds advisories | **Deferred** — this version: ic4 ⇄ ic5 and Claude-vs-local only |
...
**Covers**: `skills/metrics/emit-outcome.sh`, `skills/metrics/outcome-rates.sh`, `skills/metrics/test.sh` (or equivalent bite harness), `skills/orchestrate/SKILL.md` (Step-7 advisory sub-block + stint-end emission sub-blocks), `commands/local-do.md` (escalation emission), `specs/TDD.md` (index row) — planned/landing with CDV-185. Standup surface is SHOULD (optional).
...
- **SPEC-019** — Local-Agent Offload via OpenCode: escalation events are a ledger source; `.claude/local-agent/metrics.jsonl` is the format exemplar; token/cost accounting stays there.
