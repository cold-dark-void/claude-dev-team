...
|------|------------|----------|
| **Cold** | `/handoff <session-uuid>` | Spine-mine past session JSONL, or stripped Transcript mirror `main.md` when M3f hits; write STM packet file; **print State now + Through-line** into the invoking session and **cite the file path for appendix** (M7). |
| **Warm** | bare `/handoff` | Spine-mine **this** session's live JSONL (cold-on-self), or stripped `main.md` when M3f hits, + optional annotation; **file-only** write (M10). |
| **Help** | `/handoff --help` | Usage; exit. |

Design: deterministic LLM-free **pre-pass** (fork-tree assembly + `toolUseResult` strip + dedup) produces a size-adaptive **spine**. When a Transcript mirror is `--check status=ok` for the handoff sid, prepare MAY skip that render and feed stripped `main.md` instead (**M3f**). Absence / not-ok / fork / delta / `--full` is the JSONL identity path (byte-identical to pre-CDT-216). **Spine-mine** runs one merged LLM miner (single spine read) writing through_line + state event files over the spine (or reduced spine after chunk map), plus **deterministic git** for appendix code-state. **Assemble** (LLM-free) emits the STM packet: **State now → Through-line → appendix**, with **single-section** event render (CDT-201). Warm may add **annotation** that can only reference existing event IDs (labels/rank — never invent evidence).
...
- **SPEC-036 (Transcript mirror)** — M3f MAY read `main.md` when `--check --sid` is `ok`. Cursor is not `leaf_uuid`. Recorder / M4a / M5a frozen.
...
- **M3b — Spine-mine (shared engine).** Warm and cold MUST share one spine-mine pipeline: prepass → (optional chunk map) → **one LLM miner (merged)** + **deterministic git** → **assemble**. MUST NOT leave a freeform warm essay path as a dual source of truth.
  ...
  - **Miner prompt (CDT-201):** `skills/handoff/SKILL.md` and `skills/handoff/LIGHT.md` merged-miner templates MUST both include: (d) ruling-context — a bare pick (`1`, `y`, `B Y`) MUST NOT be the sole ruling body; `text` MUST carry the question plus the pick; `quote` MAY stay the verbatim pick (M6); static negative + positive examples; (e) `facet: product_surface` negatives — constraints and ticket IDs are not Product surfaces; constraints → `fact`/`decision` without that facet; ticket IDs → `open`/`decision`/`ship_gap` as appropriate; positives are named UX. Assemble MUST NOT add NLP/ticket-id facet filters; (f) wrapper-`summary` citation trap — both templates MUST state that the invent-guard is **all-or-nothing** (one uncited sentence discards the entire summary), that the split is `(?<=[.!?])\s+` with a trailing fragment counting as a sentence, and that an uncited scene-setting **opener** is the observed failure mode; static negative + positive examples required. Rationale: two consecutive real captures emitted fully-cited bodies preceded by a short uncited framing sentence, discarding otherwise-valid summaries. This is a **prompt** requirement — assemble's fail-closed behaviour in M11b is correct and MUST NOT be relaxed to salvage partial summaries.
...
- **M3e — Spawn model tiers (cost knobs).** Spine-mine LLM stages MUST use **tier aliases only**; MUST NOT pin dated model IDs (no `claude-*-20YY`, no `grok-4.x`, no `spark-llama`).
  ...
  - MUST NOT change `assemble.py` or `discover-warm.sh` behavior. `prepass.sh` JSONL identity behavior is frozen except the **M3f** consume branch (below).
- **M3f — Mirror spine consume (CDT-216).** After M1 locate + M9 freshness, `prepare` MAY skip M2 pre-pass **render** and write `plan.spine` from a stripped Transcript mirror `main.md` when **all** of the following hold. Otherwise MUST take the JSONL identity path (byte-identical spine to pre-CDT-216 for the same JSONL).
  - **Gate:** invoke install-aware `skills/transcript-mirror/transcript-sync.sh --check --sid <handoff-sid>` only (MUST NOT omit `--check`; MUST NOT pass `--transcript` — Grok prepare path is adapted JSONL, not `chat_history`). Consume only when stdout contains a line `sid=<handoff-sid> status=ok` (status token exact). Any other status (`lag` / `missing` / `in-progress`), missing helper, missing line, or fail-open skip → JSONL.
  - **Sid (OQ-K):** `<handoff-sid>` is the handoff session id — warm `discover-warm.sh` line 1; cold CLI uuid. MUST NOT remap to `assemble.py locate` descendant stem.
  - **Force JSONL (any one):** `--full` / `HANDOFF_FULL=1`; `--since-leaf` **applied** (`stats.since_leaf_applied==true`, including empty delta); copied-prefix / fork (OQ-F); stripped text empty.
  - **OQ-F — fork:** force JSONL if the source JSONL has a non-empty `forkedFrom.sessionId` string, or `<store>/<sid>/meta` has `parent:`, or the canonical file stem ≠ handoff sid. MUST NOT emit child-suffix-only `main.md` as the spine. Parent-prefix stitch from parent+child mirrors is **out this ticket** (unproven). SPEC-018 Test 1 stays the JSONL path.
  - **OQ-H hybrid OUT:** MUST NOT mix JSONL prefix + mirror suffix in one spine.
  - **Strip:** drop the `# transcript mirror` title; drop every line matching `^>\s*@` (Channel sidecar refs and nest-refs). MUST NOT read `thinking/`, `tool_result/`, `injection/`, or `agents/` bodies. MUST NOT inline sidecars.
  - **`leaf_uuid`:** still `keep_last_uuid` on the assembled JSONL timeline (`skills/handoff/leafrule.py`). MUST NOT store mirror `cursor` identity as `leaf_uuid` or in M8 cache. M1 assemble (or equivalent stream) still runs for the tip + fork detect.
  - **`plan.spine_origin`:** `"mirror"` on consume hit. **Omit** the key on the JSONL identity path (existing plan.json key set unchanged). On hit, `stats.mirror_sid` equals the `--check` sid. `source_files` remains the canonical JSONL path(s).
  - **Hosts:** Claude cold + Claude/Grok warm MAY hit. Grok cold-via-mirror is out (cold CLI remains UUID-shaped). MUST NOT merge SPEC-036 M5a into `discover-warm.sh`.
  - **PreCompact:** `precompact-capture.sh` MUST NOT consume `main.md`.
  - **CLI:** no new `/handoff` flags; no new `commands/*.md`.
  - Size-adaptive M3 still applies to the stripped `main.md` byte count.
...
- **M8 — Result cache.** Distilled STM packet MUST be cached keyed by `(session uuid + last-message uuid)` and reused until the session grows. Cache MUST live outside `memory.db` under the target `$MROOT/.claude/handoff/cache/`. Cache `leaf_uuid` is the assembled JSONL tip (M3f). MUST NOT add keys or retarget `leaf_uuid` at the Transcript mirror `cursor`. Schema (`leaf_uuid`, `packet`/`brief`, `events`, `path`, `created_at`, optional `light`) is frozen (SPEC-036 M12).
- **M8b — Delta-mine re-capture (CDT-88).** Warm re-capture MUST NOT re-mine the full session when a usable prior event set is available from the M8 cache.
  ...
  - **Warm delta path:** When warm re-capture finds cache `leaf_uuid` + non-empty `events` and is not full-forced, prepare MUST spine-mine **since** that cached leaf (`--since-leaf` internal/debug only — not user-facing CLI help). Stats (`est_tokens` / spine size) MUST reflect the **delta**, not the full transcript. When `--since-leaf` is **applied**, M3f MUST NOT consume `main.md` (JSONL delta identity). `--check status=ok` does not override an applied cut.
...
- **M19 — Detached orchestrator.** After parent parse + (warm) discover + resolve-root + cheap gates + **prepare**, the parent MUST read `plan.mode` **before** any skill Read or miner/annotation/chunk spawn.
  ...
  **MUST NOT:** new user-facing flags; serial-in-agent chunk map; change packet format, M8/M8b cache schema, light markers/honesty line, or CDT-80 target-root; unfreeze `assemble.py` / `discover-warm.sh`; unfreeze `prepass.sh` JSONL identity except M3f. Test 39 JSONL fixtures stay byte-identical modulo timestamp/filename.
...
40. **Mirror spine consume (M3f / CDT-216):** No store / `--check` not `ok` → `prepare` spine byte-identical to JSONL identity (no `spine_origin` key). `--check --sid` `status=ok` + no fork + not `--full` + since-leaf not applied → `plan.spine_origin=="mirror"`; `plan.leaf_uuid` equals JSONL `keep_last_uuid` tip (not `cursor`); spine has no `^> @` lines and no sidecar/nest bodies; `stats.mirror_sid` equals the sid. `--since-leaf` applied (incl. empty delta) → JSONL even when `--check` is `ok`. `--full` → JSONL. Copied-prefix (`forkedFrom.sessionId` / meta `parent:` / stem ≠ sid) → JSONL; Test 1 prefix fact still present (MUST NOT suffix-only). Nest-ref `> @agents/…` stripped. Grok warm `--check ok` MAY hit; `leaf_uuid` remains `<sid>-L<n>`; `discover-warm.sh` unchanged. Cold Grok non-UUID still usage-fail. `precompact-capture.sh` unchanged. M8 cache schema unchanged. No new `/handoff` flags. Coverage: `skills/handoff/mirror-spine-test.sh`; existing `delta-prepare-test.sh` / `discover-warm-test.sh` / `grok-to-claude-jsonl-test.sh` unweakened. SPEC-036 M4a + M5a suites still green.
...
- [ ] Detached orchestrator (M19): stub ≤12000 bytes; parent prepare then branch on `plan.mode`; `mode=direct` one background agent Reads skill from disk, IS miner (+ annotation if bare warm), no nested Task; chunked / no-spawn-host → in-session parallel map; discover-before-delegate; no new flags; engines frozen except M3f
- [ ] Mirror spine consume (M3f): `--check status=ok` MAY feed stripped `main.md`; absence/not-ok/fork/delta/`--full` JSONL identity; `leaf_uuid` stays assembled tip; `plan.spine_origin` only on hit; no CLI flags; PreCompact + M8 schema + `discover-warm.sh` + `leafrule.py` unchanged
