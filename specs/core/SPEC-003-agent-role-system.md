...
- MUST support exactly 7 agents for directives: pm, tech-lead, ic5, ic4, devops, qa, ds
- MUST store directives as numbered list (1, 2, 3... sequential, no gaps) in `.claude/memory/<agent>/directives.md` — holistic rewrite renumbers on each adjustment
- MUST surface conflicts between new and existing directives before rewriting
- MUST rewrite entire directives file holistically (never append)
- MUST treat directives as standing orders that agents cannot override — agent flags conflicts rather than ignoring
