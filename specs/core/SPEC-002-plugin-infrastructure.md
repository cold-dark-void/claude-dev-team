...
- **Preferred compound for index lookup (CDT-167 + CDT-163):** the hook MUST derive preferred compound basename `P` (no `.json`) as: basename of a candidate with `requires_council: true` when one exists; otherwise basename of any compound candidate; otherwise empty (treat as no distinct preferred — bare id only). When `P` equals the resolved task id `T`, treat as no distinct preferred.
...
- The hook template in `skills/init-orchestration/SKILL.md` (TaskCompleted body) MUST implement this resolution. Live `.claude/hooks/task-completed.sh` is generated — operators MUST re-run `/setup orchestration` after pull to regenerate (template SoT / CDT-54). Deterministic multi-meta preference when ≥2 true-council compounds exist is **out of scope** (follow-up; AC11 deferred).
