...
- **Stanza verification gate (CDT-232):** `skills/plugin-dir-test.sh` is the harness that proves the stanza's fallback branches resolve. It MUST run in the same blocking gate set as `skills/skill-lint/check-skill-bash.sh` — wired as a sibling job in `.github/workflows/smoke.yml` AND as a pre-commit gate step in `skills/release/SKILL.md`. Non-zero exit MUST block the commit, the tag, and the merge. The harness MUST extract the canonical stanza from this document's fenced block at runtime and execute that exact text; it MUST NOT hold a hand-copied or re-quoted duplicate (a second copy is the drift class SPEC-021 C5 exists to prevent, and an exempted copy is a gate that does not compose). If the canonical block cannot be located, or does not yield exactly one `PDH=$( {` line, the harness MUST fail loudly — a silent pass would make the gate vacuous.
...
Measured at **v1.18.2: 50 caller files, 207 emissions, all byte-identical** (`grep -rh --exclude-dir=fixtures … | sed 's/^ *//' | sort -u | wc -l` → `1`); 206 emissions carry the long-form `# lint-ok: C3 — marketplace …` waiver, the 207th (`commands/handoff.md:89`) carries the short form `# lint-ok: C3`. Both commands exclude `fixtures/` directories (e.g. `skills/skill-lint/fixtures/c5-pdh-drift.md`, a deliberately-drifted stanza copy used to bite-test SPEC-021 C5) — without the exclusion the raw grep over-counts. **CDT-232 evidence correction:** a prior audit reported "3 stanza variants" and read it as unwaived drift. There was none. The extra reads were (a) `skills/plugin-dir-test.sh`'s deliberately re-quoted `bash -c` copy, exempted by SPEC-021:51 and **removed** by CDT-232 in favour of runtime extraction from this document, and (b) hook-runtime bootstraps carrying `# lint-ok: C5 (hook-runtime bootstrap, not the caller-site stanza)` at `skills/init-orchestration/SKILL.md` and `skills/transcript-mirror/hook-shim.sh`. `check-skill-bash.sh` reported `300 findings, 300 waived`, exit 0. This is precisely the failure mode the sentence above warns about: the bad numbers came from hand-counting instead of letting C5 own completeness mechanically. Earlier revisions of this table claimed 11, then 15, then 26/110 — the counts above are a dated measurement, not a contract. Caller families:
...
| 16 | `skills/wrap-ticket/SKILL.md` | `WT_LIB=$(… file skills/worktree-lib.sh)`; `SIDECAR_CLI=$(… file skills/ci-watch/sidecar.sh)`; `CLOSE=$(… file skills/backlog/close.sh)`; `PRUNE=$(… file skills/wrap-ticket/prune-remote.sh)` | soft (cleanup never halts) |
...
- Stanza fallback branches (CDT-232) — cwd outside any dev checkout, synthetic `$HOME`: branch 1 (`[ -f skills/plugin-dir.sh ]`) MUST be proven bypassed (asserted, not assumed); branch 2 (marketplace) and branch 3 (version-ranked cache) MUST each resolve; final `1.0.0` MUST outrank `1.0.0-pre.N`; equal `<VER>` MUST prefer `…/cache/cold-dark-void/dev-team/…`; with no root anywhere `$PDH` MUST be empty and `bash "$PDH/skills/plugin-dir.sh" file <relpath>` MUST exit non-zero with empty stdout (SPEC-002 "no new failure path" claim, proven)
- Harness gate wiring (CDT-232) — `bash skills/plugin-dir-test.sh` MUST appear as a blocking step in both `.github/workflows/smoke.yml` and `skills/release/SKILL.md`
...
| 2026-08-31 | CDT-232: stanza verification gate — plugin-dir-test.sh wired into smoke.yml + release pre-commit gates; harness now executes the canonical stanza extracted from this document instead of a re-quoted copy; emission counts re-measured (50 files / 207 emissions / 1 variant). |
