...
- When `effective_requires_council` is true, the hook MUST then query `.claude/council/index.json` for a qualifying row and apply the council gate logic below: dual-shape pass signal (non-null `max_verdict_confidence` **or** null verdict conf + non-null `max_finding_confidence`; effective score = max across resolved rows) ≥ `council.taskgate.min_confidence`, distinct fail modes
- For index compound-key fallback (below), the hook MUST prefer the basename (no `.json`) of a candidate with `requires_council: true` when one exists; otherwise the basename of any compound candidate; otherwise the bare id `T`
...
- The hook MUST parse the index as `{ "<task_id>": [ { "report_path": string, "max_verdict_confidence": int|null, "max_finding_confidence": int|null, "created_at": ISO-8601 }, … ], … }`. Index key resolution MUST mirror task-metadata compound-key fallback (CDT-122): try bare stdin/`CLAUDE_TASK_ID` key first, then the compound basename of the resolved task-metadata file (e.g. `CDT-111-C1-7` when meta is `…/CDT-111-C1-7.json` and bare id is `7`), then any index key that ends with `-<bare_id>`.
- The hook MUST accept either confidence shape as a pass signal (CDT-122): a row with non-null `max_verdict_confidence` (claim/`verdict[]` runs) **or** a row with null `max_verdict_confidence` and non-null `max_finding_confidence` (diff-mode/`finding[]` runs). The effective score for a task is the maximum of all such confidences across resolved rows; the gate passes when that score ≥ `council.taskgate.min_confidence` (default 80).
- The hook MUST hard-fail with exit code 2 and a clear stderr message in each of the following cases (no soft fallbacks, no silent passes):
  ...
  - Task id absent from the index (after compound-key fallback) → stderr "no verdict exists" naming the blocked task id
  - Task id present but no confidence row meets the threshold → stderr naming the blocked task id, the best observed confidence, and the required `council.taskgate.min_confidence` value
