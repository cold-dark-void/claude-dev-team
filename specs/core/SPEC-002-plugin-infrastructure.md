...
**Covers**: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.claude/settings.json` (install-time, gitignored), emitted project hooks under `.claude/hooks/` (generated by `/setup orchestration` from `skills/init-orchestration` templates — not tracked in the plugin package after CDT-54), `skills/init-orchestration/SKILL.md` (hook template SoT), `tools/scout-plugins/README.md`, `skills/plugin-dir.sh`
