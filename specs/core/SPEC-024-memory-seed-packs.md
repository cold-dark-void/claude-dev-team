...
- **M12 — Agent-name containment on import (normative).** A pack is untrusted input (M8), and import turns two pack-supplied strings into paths: a manifest `files` key (joined to `SEED_DIR`) and an entry trailer's `agent=` field (joined to `$MROOT/.claude/memory/` on the M10 fallback write). Both MUST be validated against the fixed agent roster — the set `seed_agents()` returns — before they are used to construct any path, and therefore before any existence check, content-hash comparison, `mkdir`, or append. A manifest key MUST be exactly `<agent>.md` for a roster agent; a trailer `agent=` MUST name a roster agent. This mirrors the check export already performs on `--agent`, and it closes a path escape: `../../../../etc/foo.md` is a syntactically valid manifest key, and a trailer agent of `../../..` is a write primitive that **no hash gate can catch** — M3 computes the entry hash over content *excluding* the trailer, and a hostile pack author computes the manifest hash themselves. Roster validation MUST NOT be folded into trailer parsing: parsing establishes syntax, import applies policy, and the two have different lifetimes.
  ...
  A rejection at either sink MUST be counted in the M6 `rejected` total and reported with a one-line warning naming the roster as the reason, so a bad name is diagnosable as an authoring or version-skew error rather than reading as file corruption. Per M8 it MUST NOT abort the run: remaining files and entries still import and `/setup team` still completes. A pack produced by a newer plugin version carrying an agent this version does not know is rejected by this rule; that degradation is intended and fail-safe.
