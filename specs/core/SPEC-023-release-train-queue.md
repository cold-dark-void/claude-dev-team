...
- **SPEC-002 (plugin infrastructure)** owns the version-pair rule itself (CHANGELOG.md heading + `plugin.json` must match; `marketplace.json` is not version-synced) and the version format conventions. The train renumbers assumed version strings to the assigned slot but final pair verification remains `/release` Step 4's job.
...
- **M3 — Slot version assignment & renumbering.** Slot versions MUST be computed relative to the CURRENT master version pair at train start — never hardcoded: slot 1 applies its entry's `bump` to master's version (a feature branch defaults to `minor`, i.e. the next feature minor; `patch` is allowed per the `/release` feature-line arc policy), and each subsequent slot applies its `bump` to the previous slot's result. For an entry whose branch pinned an `assumed_version` (e.g. a `### vX.Y.Z` CHANGELOG heading and/or `plugin.json` version field), the train MUST renumber every occurrence of that assumed string on the version-pair surfaces on the merged working tree to the `assigned_version`. If a legacy branch still carries `marketplace.json` `plugins[].version`, renumber only when the field is already present — MUST NOT invent a version field on marketplace. Branches that pinned nothing need no renumbering — `/release`'s own Step-3 bump covers them.
...
- **M12 — CLI surface.** Mechanical operations MUST be exposed as a subprocess-only CLI (never sourced). Callers MUST resolve `train-lib.sh` install-aware via `plugin-dir.sh` (`TRAIN_LIB=$(bash "$PDH/skills/plugin-dir.sh" file skills/release-train/train-lib.sh)` then `bash "$TRAIN_LIB" <cmd> …`) — never a bare cwd-relative `bash skills/release-train/…` path (marketplace/cache installs). Commands:
  ...
  - `freeze [--order b1,b2,…] [--print-only]` — compute + write order + `assigned_version` from master version pair (`--print-only` prints without writing)
...
3. **Slot arithmetic & renumbering (M3):** with master at some vX.Y.Z and two queued `feat:` branches (defaults `minor` then declared `patch`), the dry-run assigns the next feature minor to slot 1 and the following patch to slot 2; a branch that pinned an assumed version has its CHANGELOG heading and `plugin.json` version renumbered to its slot on the merged tree (marketplace version only if already present), and `git log <branch>` shows the source branch unchanged.
...
6. **CHANGELOG stacking (M5c, M5d):** after both land, `CHANGELOG.md` headings descend in version order with exactly one heading per version, each branch's authored bullet content preserved under its renumbered heading, and the version pair matches (per `/release` Step 4 passing).
...
- **SPEC-002** — Plugin Infrastructure: version-pair rule and version format conventions; final pair verification stays in `/release` Step 4.
