...
- **M4 — Backlog is the source of truth.** On approval, each child MUST be written through SPEC-009's backlog conventions (`.claude/backlog/<slug>.md` item + `.claude/backlog.md` index row), carrying the five M1 fields plus the epic parent ID and `depends_on`. Child IDs MUST use the form `<EPIC-ID>-C<n>` (e.g. `CDV-30-C2`) — the `C` infix prevents collision with SPEC-017's within-ticket compound task keys (`<ISSUE-ID>-<task_id>`, e.g. `CDV-30-2`).
- **M5 — Linear mirror is informational only.** When the Linear MCP is available, `/epic` MUST attempt best-effort creation of child issues and record the returned Linear identifiers in the epic state file; the local `<EPIC-ID>-C<n>` ID remains the canonical key. When the MCP is absent or any Linear call fails, `/epic` MUST emit a one-line notice and continue — it MUST NOT block, retry-loop, or fail the epic on Linear unavailability (backlog files remain the source of truth).
...
- [ ] Linear-absent run completes cleanly with backlog as source of truth *(protocol; manual)*
...
| L2 | **Linear is optional degrade** — backlog files are SoT; one-line notice on MCP fail; local IDs `<EPIC-ID>-C<n>` canonical. |
