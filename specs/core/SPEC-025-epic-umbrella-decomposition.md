...
- **M14 — Integration worktree + end-release CLI flags (CDT-141).** `/epic` MUST accept opt-in flags that record *intent* for an integration worktree and a single end-of-epic release. **C1** = parse + durable state. **C2** = ensure one `epic-<ID>` integration worktree when `worktree_enabled`. **C3** = every child `/kickoff`/`/orchestrate` MUST use that integration worktree only (`ensure-ticket-worktree` skips per-child `worktree-lib ensure`); B.4 handoff MUST carry path + no-per-child-WT instruction; `/wrap-ticket` for a child MUST NOT release the integration slug. **C4** = mid-epic `/release` + master-merge forbid while `release_bump` is set and seal is not done (`assert-release-allowed`; exit 64; message names release=end until seal). **C5** = end-of-epic seal after last child completed (`seal-ready` / `seal`): squash-stage integration onto master/main, exactly one `/release <release_bump>` with `EPIC_ALLOW_SEAL_RELEASE=1`, then `sealed=true`; failure leaves `sealed=false` and master clean. **C6** = resume reuses same integration tree/branch from state; flag-vs-state conflict policy (below). **C7** = SPEC + surface docs + regression bite-tests for the locked CLI (this section). **M11 still holds** with carve-outs only for ensure/route of the integration worktree and seal composition — no full orchestrate lifecycle reimplementation.
  ...
  13. **End-of-epic seal (C5 / CDT-141-C5 / CDT-170).** When durable `release_bump` is non-null, after **all** children are `completed` and `sealed` is not true, Mode B MUST run the seal path **once**:
      ...
      - `epic-lib seal <EPIC-ID>` squash-stages the integration branch onto master/main (**no** commit; reversible with `git reset --hard` of **seal-owned** stage only after a clean precheck). Then the orchestrator invokes **exactly one** `/release <release_bump>` (bump from durable state) with `EPIC_ALLOW_SEAL_RELEASE=1`. `/release` remains the sole ship-of-record (version pair + one fold-commit + tag/push). On success: `seal --complete` sets `sealed=true` atomically.
      - **Abort dirty gate (CDT-170):** Define **dirty** as non-empty `git status --porcelain` on the main-repo checkout (`SEAL_MAIN` / `$MROOT` bare checkout that owns master/main).
        - `seal <EPIC-ID> --abort` **without** `--force` MUST, when dirty: exit **1**, print a stderr message that names **dirty** and **refuse abort** (or equivalent refuse wording), perform **zero** `git reset --hard` and **zero** `git clean -fd`, leave the working tree **byte-identical**, and leave `sealed` **unchanged**. This MUST apply even when `already_sealed=true` — dirty check MUST run **before** any reset.
        - `seal <EPIC-ID> --abort` when **clean**: exit **0**; keep existing abort JSON shapes (`aborted` / `already_sealed`); MUST NOT flip `sealed` to true. Reset/clean of an already-clean tree is allowed (no-op effect).
        - `seal <EPIC-ID> --abort --force` MAY hard-reset + clean when dirty (operator/orchestrator recovery only). `--force` is legal **only** with `--abort`; `--force` alone or with `--dry-run`/`--complete` MUST exit **64** (usage). Unknown flags still exit **64**.
        - **Same-invocation recovery (MUST):** on squash fail or release-hook fail after live seal started from a **clean** tree, `_seal_reset_main` (or equivalent) MUST still restore the seal-owned stage so master is clean of partial squash; `sealed` stays false; **no** partial release tag/push from the seal path. That recovery path is **not** gated by the abort dirty refuse (seal already owned the tree after the clean precheck).
      - Re-invoke after `sealed=true` is a no-op (`already_sealed`) when clean (or with `--force` if dirty wipe is intended). Mid-epic (incomplete children) MUST NOT seal (exit 64). Master MUST remain free of epic delivery commits until seal succeeds.
