...
### Bump-class gate (new command or unflagged-skill surface)

Fail-closed: a newly added `commands/*.md` that does **not** wrap a skill
already on the old ref, or a newly added `skills/<name>/SKILL.md` whose YAML
frontmatter lacks `user-invocable: false`, is a new user-facing Surface and MUST
ship as **minor or major**, never patch (AGENTS.md versioning). This is the
1.7.37 class of defect (`/audit` tagged as a patch). A new `commands/<name>.md`
MUST NOT count when `skills/<name>/SKILL.md` exists at the old ref (thin host
door so TUIs that only load `commands/` can reach an engine that already
shipped). Flagged skills stay patch-eligible.

- **B1 — Deterministic checker CLI.** MUST ship `skills/release/check-bump-class.sh` as pure-subprocess bash (no LLM, no network). Exit codes: `0` = ok (no new Surface, or bump is minor/major); `1` = new `commands/*.md` or unflagged `skills/<name>/SKILL.md` with patch / unchanged / unreadable version; `64` = usage / not a git repo. Modes: default = worktree+index+untracked vs `HEAD` (or `--against REF`); `--cached` = index vs HEAD (pre-commit); `--commit REV` = that commit vs its parent (CI). Usage text MUST name both command files and unflagged skills.
- **B2 — Predicate.** Collect added paths matching `commands/*.md` or `skills/<name>/SKILL.md` (`--diff-filter=A`, plus untracked in default mode). Drop a `commands/<name>.md` from the set when `skills/<name>/SKILL.md` exists at the old ref (`git cat-file -e <old>:<skill>`). A new skill counts iff its YAML frontmatter does not contain whitespace-tolerant `user-invocable: false` (optional quotes). Missing or unreadable frontmatter counts as unflagged. If the Surface set is empty → pass. Else read `plugin.json` `"version"` at the old ref and the new tree; classify the pair as `major` / `minor` / `patch` / `none` / `invalid`. Pass iff class ∈ {`minor`, `major`}. Edits or deletes of existing command or skill files MUST NOT trip the gate. A new flagged skill (`user-invocable: false`) MUST NOT trip the gate. A thin command door over a pre-existing skill MUST NOT trip the gate.
- **B3 — Fail message.** On policy fail print `bump-class:`, list every new Surface path, print `old -> new (class)`, cite AGENTS.md, and state commit/tag/push MUST NOT proceed. MUST NOT mutate the index.
...
- **B5 — Tests.** MUST ship `skills/release/test-bump-class.sh` in temp repos: new command + patch → 1; new command + minor/major → 0; edit existing + patch → 0; new flagged skill + patch → 0; new unflagged skill + patch → 1; thin command over a skill already on the old ref + patch → 0; `--commit` and `--cached` variants; usage → 64.
- **B6 — MUST NOT.** Enforce on feature branches; treat a new `skills/<name>/SKILL.md` with `user-invocable: false` as a Surface; treat a `commands/<name>.md` that wraps a skill already on the old ref as a new Surface; allow a feature-line `/release patch` to add a **new** command file (no matching baseline skill) or unflagged skill (those stay minor/major).
...
- **D11 — README surface tokens (`surface-readme`).** MUST verify every backticked `/name` token (`/` + `[a-z0-9-]+` inside backticks) in repo-root `README.md` resolves to `commands/<name>.md` **or** a `skills/<name>/SKILL.md` whose YAML frontmatter does **not** set `user-invocable: false`. Skip: fenced code blocks (D8); the renamed/removed table whose header row is `Old command` (both columns). Host-only names that are not plugin Surfaces MUST NOT be written as backticked `/name` tokens. D6 waivers apply.
- **D12 — Skill surface pairing (`surface-skill`).** MUST verify every `skills/<name>/SKILL.md` that does not set `user-invocable: false` in YAML frontmatter (frontmatter-only; same token as `/doctor` `skills.user_invocable`: `false` / `"false"` / `'false'`) has a matching `commands/<name>.md`. Missing flag means user-invocable. Skills without `SKILL.md` are out of scope. D6 waivers apply.
...
- Verify bump-class gate via `bash skills/release/test-bump-class.sh`: new `commands/*.md` + patch → 1; + minor/major → 0; new flagged skill + patch → 0; new unflagged skill + patch → 1; thin command over a baseline skill + patch → 0
...
11. **README surface tokens (D11):** inject a ghost `` `/no-such-cmd` `` in README prose outside the renamed/removed table → exit `1` with `[surface-readme]`; the same token only inside the `Old command` table → no `surface-readme` finding; a `/name` backed by `commands/<name>.md` or an unflagged skill → no finding.
12. **Skill surface pairing (D12):** inject an unflagged `skills/<name>/SKILL.md` with no `commands/<name>.md` → exit `1` with `[surface-skill]`; `user-invocable: false` and no command file → no finding; unflagged skill with a matching command file → no finding.
...
| Date | Change |
|------|--------|
| 2026-09-18 | Bump-class B2: a new `commands/<name>.md` is not a Surface when `skills/<name>/SKILL.md` exists at the old ref (thin host door for TUIs that only load `commands/`). |
| 2026-09-17 | CDT-251: bump-class B1–B6 — new unflagged `skills/<name>/SKILL.md` (frontmatter lacks `user-invocable: false`) requires minor/major, same as new `commands/*.md`. Flagged skills stay patch-eligible. Missing/unreadable frontmatter = unflagged. |
| 2026-09-17 | CDT-256: D11 `surface-readme` + D12 `surface-skill` — README `/name` tokens and unflagged skills must pair with `commands/<name>.md` (or an invocable skill for D11). D1 check-id list extended; D2–D10 unchanged. |
