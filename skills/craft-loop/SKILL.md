...
- NEVER invoke `/loop` or `/goal` yourself. Your terminal output is the saved
  program plus its invocation line; the user fires the loop.
...
- `.claude/loops/<name>.md` — programs
- `.claude/loops/<name>.journal.md` — written by the running loop
...
2. **Guided dialogue — one question per message.** Cover, at minimum, in
   ...
   - **Cadence** — `dynamic` (self-paced) or a fixed interval suggestion
   - **Target** — `/loop` for discrete repeated work; `/goal` for a standing
     objective without discrete firings
   - **Name** — kebab-case, unique in `.claude/loops/`
...
3. **Draft.** Copy the template's fenced body; seed the procedure from
   ...
   (the code path and invocation line for goal MUST be present even when live
   acceptance exercises `/loop` only).
4. **Quality checklist — every item must pass before you present:**
   ...
   6. One iteration is small enough for a single firing — **human judgment
...
6. **On approval:** write `.claude/loops/<name>.md` (create the directory if
   absent), then print the **exact** invocation line for the chosen target
...
1. Enumerate the library (self-contained block; find-based, no globs):

```bash
WTROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
LOOPS_DIR="$WTROOT/.claude/loops"
if [ -d "$LOOPS_DIR" ]; then
  find "$LOOPS_DIR" -maxdepth 1 -type f -name '*.md' -not -name '*.journal.md' | sort
else
  echo "NO_LIBRARY"
fi
```

2. `NO_LIBRARY` → say the project has no crafted loops yet and stop.
