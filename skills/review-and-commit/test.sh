#!/usr/bin/env bash
# skills/review-and-commit/test.sh — CDT-258 static contract suite for the review-and-commit skill.
...
# Run: bash skills/review-and-commit/test.sh
set -u
...
SKILL="$HERE/SKILL.md"
CMD=""
SPEC="$ROOT/specs/core/SPEC-013-adversarial-council-tribunal.md"
...
# ---- Surfaces present -------------------------------------------------------
if [ -f "$SKILL" ]; then ok "skills/review-and-commit/SKILL.md exists"; else bad "skills/review-and-commit/SKILL.md missing"; fi
if [ -n "$CMD" ]; then
  if [ -f "$CMD" ]; then ok " exists"; else bad " missing"; fi
fi
if [ -f "$SPEC" ]; then ok "specs/core/SPEC-013-adversarial-council-tribunal.md exists"; else bad "specs/core/SPEC-013-adversarial-council-tribunal.md missing"; fi
...
has "$SKILL" '^name:[[:space:]]*review-and-commit' "skill name: review-and-commit"
[ -n "$CMD" ] && has "$CMD" '^name:[[:space:]]*review-and-commit' "command name: review-and-commit"
...
# ---- review-and-commit structural contracts ---------------------------------
has_f "$SKILL" "$MARKER" "review-and-commit self-verified marker"
has_f "$SKILL" 'diff-mode' "review-and-commit diff-mode preset"
has_f "$SKILL" 'confidence 80' "review-and-commit confidence 80 filter"
has_f "$SKILL" '## Step 2: Locate the council engine' "review-and-commit locates council engine"
has_f "$SKILL" '## Step 7: Commit Gate' "review-and-commit Step 7 commit gate"
has_f "$SKILL" 'skills/council/engine.sh' "review-and-commit cites council engine.sh"
has_f "$SKILL" 'skills/council/SKILL.md' "review-and-commit cites council SKILL"
has_f "$SKILL" 'skills/security-scan/SKILL.md' "review-and-commit cites security-scan"
has_f "$SKILL" 'SPEC-013' "review-and-commit cites SPEC-013"
