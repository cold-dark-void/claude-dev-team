...
`skills/spec-tooling/spec-skeleton.md` (single-sourced via the `<!-- include -->` region
below — SPEC-008). Do NOT hand-edit the region between the markers; run
`python3 skills/agent-memory/sync-includes.py apply skills/generate-specs/SKILL.md` to
refresh it. The emitter EXTRAS (`**Covers**:`, `## SHOULD`, `## Open Questions`,
...
- Render the literal `<STATUS>` token in the template as:
  `INFERRED — generated by /generate-specs on <YYYY-MM-DD>. Requires human review.`
