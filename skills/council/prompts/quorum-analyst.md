...
name: quorum-analyst
description: |
  Blind-path (`/council --blind`) quorum analyst prompt. Reads all FINDING-NNN
  blocks from all reviewer teams, clusters semantically similar findings,
  assigns quorum tiers, and outputs a ranked CLUSTER list. NOT blind — sees
  all teams' findings by design. Returns structured CLUSTER-NNN blocks only.
---

# quorum-analyst prompt template

Runtime template for the quorum analyst agent on the `--blind` path.
`commands/council.md` substitutes `{{ALL_FINDINGS}}`, `{{TEAM_MANIFEST}}`,
`{{UNCONSTRAINED_TEAMS}}`, `{{LENS_TEAMS}}`, `{{TOTAL_TEAMS}}` before spawning
the single Task call.
...
Source-findings: [comma-separated namespaced finding IDs, e.g. U1-FINDING-003, L-security-FINDING-001]
...
|---|---|---|
| `{{ALL_FINDINGS}}` | string | orchestrator — namespaced FINDING blocks by team |
| `{{TEAM_MANIFEST}}` | string | orchestrator — team IDs + type (unconstrained/lens) |
