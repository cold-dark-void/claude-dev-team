...
- Emitter EXTRAS live entirely AFTER the closing `<!-- /include -->` marker, never inside:
    * /spec generate: `**Covers**: <files>` line, then `## SHOULD`, `## Open Questions`,
      ...
      PRESENCE + spelling + heading-level, NOT order. /spec check Phase 1 passes.)
    * /spec create: emits NO extras; its `---` separator style stays OUTSIDE the markers
      (above/below), so the included bytes match /spec generate exactly.
- The `**Covers**:` line is NOT one of the 9 required sections (/spec check does not check it).
  It is emitter metadata; /spec generate emits it as an extra after the region.
