...
Protocol for `/memory distill`. Thin command host: `commands/memory.md`.
Optional prose compress: `skills/memory-compress/SKILL.md`.
Pre-distill validate: `skills/validate-memory/pipeline.md` (Steps 1–8, no `--deep`).
...
2. **Run `/memory validate $VALIDATE_ARGS` now** — follow
   `skills/validate-memory/pipeline.md` (Steps 1–8). Do NOT pass `--deep`
   (circular dependency: distiller → validate → distiller).
