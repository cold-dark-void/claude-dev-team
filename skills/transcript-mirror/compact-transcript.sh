#!/usr/bin/env bash
# compact-transcript.sh — Meaning-tail CLI (SPEC-036 M14).
...
if [ -z "${PDH:-}" ] || [ ! -f "$PDH/skills/plugin-dir.sh" ]; then
  echo "compact-transcript: dev-team plugin not found" >&2
  exit 1
fi

COMPACT_PY=$(bash "$PDH/skills/plugin-dir.sh" file skills/transcript-mirror/compact-transcript.py 2>/dev/null) || COMPACT_PY=""
if [ -z "$COMPACT_PY" ] || [ ! -f "$COMPACT_PY" ]; then
  echo "compact-transcript: compact-transcript.py not found" >&2
  exit 1
fi

if ! command -v python3 >/dev/null 2>&1; then
  echo "compact-transcript: python3 not found" >&2
  exit 1
fi

exec python3 "$COMPACT_PY" "$@"
