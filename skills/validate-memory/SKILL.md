...
Used in Step 4 Tier B of `/memory validate`. The orchestrating command reads
...
Used by `/memory validate --reconcile` Step R1. Host generates candidates via
...
Used in Step R2 of `/memory validate --reconcile`. The orchestrating command
