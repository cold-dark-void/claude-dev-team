...
require_int() {
  case "$2" in
    ''|*[!0-9]*) echo "reconcile-lib.sh: $1 must be an integer, got '$2'" >&2; exit 64 ;;
  esac
}
...
jaccard() {
  ...
  local ta_f tb_f
  ta_f=$(mktemp "${TMPDIR:-/tmp}/jac_a.XXXXXX")
  tb_f=$(mktemp "${TMPDIR:-/tmp}/jac_b.XXXXXX")
  # shellcheck disable=SC2086
  printf '%s\n' $ta | sort -u >"$ta_f"
  # shellcheck disable=SC2086
  printf '%s\n' $tb | sort -u >"$tb_f"
  local inter union
  inter=$(comm -12 "$ta_f" "$tb_f" | wc -l | tr -d ' ')
  union=$(sort -u "$ta_f" "$tb_f" | wc -l | tr -d ' ')
  rm -f "$ta_f" "$tb_f"
...
_log_reconcile() {
  ...
  require_int "id_a" "$id_a"
  require_int "id_b" "$id_b"
  require_int "confidence" "$conf"
  [ -n "$winner" ] && require_int "winner" "$winner"
  [ -n "$loser" ] && require_int "loser" "$loser"
...
cmd_resolve_pick() {
  ...
  require_int "winner" "$winner"
  require_int "loser" "$loser"
  sqlite3 -cmd ".timeout 5000" "$memdb" "PRAGMA busy_timeout=5000;
