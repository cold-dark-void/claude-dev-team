# /memory validate — host pipeline

Prompt templates, claim/verdict taxonomies, composite scoring, batching
limits, and pair-judge contracts live in `SKILL.md` (same directory).
Load those sections when steps below reference them. This file is the
host pipeline (absorbed from the former `/validate-memory` command).

Cross-reference agent memories against the live codebase; `--reconcile`
detects cross-agent contradictions. Detect and resolve stale references —
dead files, renamed functions, shifted line numbers, outdated factual claims.
Uses LLM-based per-claim extraction and two-tier verification (bash for
structural refs, LLM investigator for semantic claims) with confidence
scoring: validator proposes, tech-lead reviewer confirms, user decides
ambiguous cases.
