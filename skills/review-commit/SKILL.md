...
1. **Reset review file** — Overwrite `/tmp/review.md` with a fresh file (do
   not append). Previous feedback is assumed to have been addressed. Start
   fresh each run.
...
4. **Write to /tmp/review.md** — Write a dated section with findings.
...
5. **Commit** — If clear, commit with a conventional commit message
   that summarizes *why* the change was made. The review file lives in
   `/tmp/` and must never be staged or committed.
